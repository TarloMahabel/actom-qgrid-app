-- =====================================================================
-- Migration 008 — authorisation bypass via NULL role comparison
--
-- SEVERITY: high. Fix before the next intake opens.
--
-- THE BUG
--
...
--       end if;
--
--   app.reviewer_role() returns NULL for anyone who is not an active
--   reviewer — which is every applicant, and every signed-in person who
--   has not been activated. In SQL:
--
--       NULL not in ('admin','manager')  ->  NULL
--
--   and `if NULL then ... end if` does not fire. So the authorisation
--   check was skipped entirely for exactly the people it was written to
--   stop. The same applies to `<>` and to `is distinct from` used the
--   wrong way round.
--
--   Impact: a signed-in applicant could reach publish_intake,
--   close_intake, clone_intake, save_trade_subjects, enrol_applicant,
--   update_apprentice, unenrol_apprentice and mark_storage_purged.
--   Several then failed for unrelated reasons — a lookup returning no
--   rows, a required argument — which is luck, not access control.
--
--   Found by the adversarial suite (db/test/91-adversarial.sql), which
--   attempts each of these as a real applicant.
--
-- THE FIX
--
--   One helper that raises, used everywhere. It cannot be written
--   incorrectly at the call site, and NULL is handled once.
--
-- Run after 007-apprentice-register.sql. Idempotent.
-- =====================================================================
