-- =====================================================================
-- Migration 002 — Configurable intake forms
--
-- Run after schema.sql and seed.sql. Idempotent.
--
-- DESIGN
--   `trades` and `subjects` remain a global catalogue that HR can edit
--   freely at any time. What an applicant actually sees is a per-intake
--   SNAPSHOT of that catalogue, held in intake_* tables. Editing the
--   catalogue therefore can never disturb an intake that is already
--   taking applications — which is the whole point of the lock.
--
--   Lifecycle:  draft ──publish──> open ──close──> closed ──> archived
--                 ^                  |
--                 |                  └── config is FROZEN from here on
--                 └── clone_intake() to start the next one
--
--   Freezing is enforced by triggers, not by the UI, so it holds even
--   against a direct SQL session.
-- =====================================================================
