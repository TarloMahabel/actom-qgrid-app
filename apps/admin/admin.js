...
(function () {
  ...
  var APP_COLS = [
    ...
    'qual_codes','qual_points','qual_highest','qual_note','qual_source'
  ].join(',');
  ...
  var NAV = [
    ...
    { key: 'consent', label: 'Consent wording', icon:
  ...
  function allowed(key) {
    ...
    if (key === 'consent')   return ['admin','information_officer'].indexOf(S.me.role) >= 0;
  ...
  /* Help. Written as a walk through the actual job rather than a list
     of features, and gated by role so nobody reads about a screen they
     cannot open. Update this whenever a section is added — a stale help
     page is worse than none, because it is believed. */
  function showHelp() {
    var role = S.me.role;
    var canSetup   = ['admin','manager'].indexOf(role) >= 0;
    var canConsent = ['admin','information_officer'].indexOf(role) >= 0;
    var canAudit   = ['admin','manager','information_officer'].indexOf(role) >= 0;
    var canEnrol   = ['admin','manager'].indexOf(role) >= 0;

    function card(eyebrow, title, body) {
      return '<div class="card"><div class="card-head"><div class="eyebrow">' + eyebrow +
             '</div><h2>' + title + '</h2></div>' + body + '</div>';
    }

    $('content').innerHTML =

      '<div class="card"><div class="card-head"><div class="eyebrow">Help</div>' +
      '<h1>Using the reviewer console</h1>' +
      '<p>You are signed in as <strong>' + esc(role.replace(/_/g, ' ')) + '</strong>. ' +
      'This page shows only the parts of the system your role can reach.</p></div>' +

      '<h3>The short version</h3>' +
      '<ol style="line-height:1.7;padding-left:1.2rem">' +
      '<li><strong>Dashboard</strong> — where the intake stands overall.</li>' +
      '<li><strong>Applications</strong> — the queue. Open one, read it, decide.</li>' +
      '<li>Decide: <em>Shortlist</em>, <em>Mark under review</em>, or <em>Not selected</em>.</li>' +
      (canEnrol ? '<li>Shortlisted and appointed? <em>Enrol as apprentice</em>.</li>' : '') +
      '<li><strong>Apprentices</strong> — everyone taken on, and how far through they are.</li>' +
      '</ol>' +
      '<p class="small muted" style="margin-bottom:0">The menu is the button at the top left. ' +
      'Pin it open with the pin icon if you would rather it stayed.</p></div>' +

      card('Dashboard', 'Where the intake stands',
        '<p>The first screen you see. It shows how many applications have arrived, how many ' +
        'are still waiting for a first look, and how each trade is filling up.</p>' +
        '<p>The bar per trade turns <strong>red</strong> when fewer applicants meet the ' +
        'requirements than there are positions — that is the number worth watching, because ' +
        'it tells you early whether a trade will actually fill.</p>' +
        '<p>Click <strong>Awaiting review</strong> or <strong>Shortlisted</strong> to jump ' +
        'straight into the queue already filtered.</p>' +
        '<p class="small muted" style="margin-bottom:0">Deliberately shows no names. It is the ' +
        'first thing open on your screen, often where others can see it.</p>') +

      card('Applications', 'Working the queue',
        '<p>Filter by status, trade and intake along the top; search by name, reference, the ' +
        'last four of an ID, or a phone number. Click anywhere on a row to open it.</p>' +
        '<p><strong>Top N by score</strong> is for shortlisting meetings. Enter a number, or ' +
        'use <em>Top 100</em>. It sorts by score and shows the cut-off, plus how many people ' +
        'tied on that exact score just outside the cut — so you can see when the line fell in ' +
        'an awkward place. Choose a single trade first, or you get everyone mixed together.</p>' +
        '<p>Applications without a score are left out of that ranking and counted separately, ' +
        'rather than quietly taking a place in the top hundred.</p>') +

      card('Deciding', 'Shortlist, not selected, and enrolment',
        '<p><strong>Shortlist</strong> and <strong>Mark under review</strong> are ordinary ' +
        'status changes. Notes are optional but useful.</p>' +
        '<p><strong>Not selected</strong> opens its own dialog and asks for a reason from a ' +
        'set list — below the minimum requirements, position filled, failed the assessment, ' +
        'incomplete documents, or unreachable. That is not box-ticking: it is what lets ACTOM ' +
        'answer later where in the process candidates are being lost, per trade.</p>' +
        '<p>Changing a decision you have already recorded needs a reason in the notes. The ' +
        'change is kept, along with who made it.</p>' +
        (canEnrol
          ? '<p><strong>Enrol as apprentice</strong> appears once someone is shortlisted. It ' +
            'asks for the start date, employee and SETA numbers, site and supervising artisan. ' +
            'Only the start date is required — the SETA number usually only arrives once the ' +
            'contract is registered, and you can add it later from the register.</p>' +
            '<p class="small muted" style="margin-bottom:0">Enrolling is lasting: the ' +
            'application becomes an employment record and stops being subject to the 12-month ' +
            'applicant retention. Only an administrator can undo it.</p>'
          : '<p class="small muted" style="margin-bottom:0">Enrolment is done by a manager or ' +
            'administrator.</p>')) +

      card('Scoring', 'How the numbers are worked out',
        '<p>A score is the <strong>weighted average</strong> of the marks for the subjects ' +
        'configured for that trade. A subject with weight 3 counts three times as much as one ' +
        'with weight 1. Weight 0 captures the mark without scoring it.</p>' +
        '<p>A required subject left blank scores <strong>zero</strong> rather than being ' +
        'skipped — so missing a compulsory subject costs the applicant, as it should.</p>' +
        '<p>Applicants are only ever scored against the certificate they actually sat. Someone ' +
        'who wrote the academic NSC is never marked down for missing technical subjects.</p>' +
        '<p><strong>Flags are advisory.</strong> Nothing is ever declined automatically. Every ' +
        'decision is made by a person, and recorded against their name.</p>' +
        (canSetup ? '<p class="small muted" style="margin-bottom:0">Form setup has a fuller ' +
                    'explanation with a worked example, right above where you set the weights.</p>' : '')) +

      card('Documents', 'Opening an applicant\u2019s files',
        '<p>Documents are virus scanned before anyone can open them. Until a file is cleared it ' +
        'shows as <em>not yet</em> and cannot be opened; a file that fails is marked ' +
        '<strong>quarantined</strong> and stays blocked.</p>' +
        '<p>If files sit unscanned for a long time the scanner has probably stopped — tell ' +
        'Group IT rather than working around it. An administrator can release a specific file ' +
        'with a written reason, which is recorded.</p>' +
        '<p class="small muted" style="margin-bottom:0">Opening a document is logged, as is ' +
        'unlocking an ID number. Unlocking asks why, and the reason is kept.</p>') +

      card('Apprentices', 'The register',
        '<p>Everyone taken on from an intake. Filter by active, completed, withdrawn or ' +
        'terminated. The progress bar shows how far through the contract each person is.</p>' +
        '<p>Open an entry to record a change: trade test date and result, site, supervisor, ' +
        'and the <strong>employee and SETA learner numbers</strong> once they come through.</p>' +
        '<p>Ending an apprenticeship needs both a date and a reason. The record stays on the ' +
        'register either way — people who left are part of the history, not deleted from it.</p>' +
        '<p class="small muted" style="margin-bottom:0">These records outlive applications. ' +
        'They are employment records and are kept accordingly.</p>') +

      (canSetup ? card('Form setup', 'Configuring an intake',
        '<p>Set the trades on offer, the positions per trade, the subjects and their weights, ' +
        'minimum marks, and which documents are asked for.</p>' +
        '<p><strong>Publishing freezes the form permanently.</strong> That is deliberate: the ' +
        'form an applicant filled in has to stay exactly as they experienced it. To change ' +
        'anything afterwards, clone the intake and publish the clone.</p>' +
        '<p>Publishing checks the intake is complete first — at least one trade switched on, ' +
        'the ID document required, sensible dates, and consent wording that exists.</p>') : '') +

      (canConsent ? card('Consent wording', 'What applicants agree to',
        '<p>The POPIA consent text an applicant ticks before submitting. Each version carries ' +
        'separate wording for the applicant and for the parent or guardian of an under-18 — ' +
        'both are required before an intake can be published.</p>' +
        '<p><strong>Wording locks as soon as one applicant has agreed to it.</strong> The ' +
        'record has to show what that person actually saw, so it cannot be reworded afterwards. ' +
        'Use <em>New version</em> instead; it pre-fills from the current wording.</p>' +
        '<p class="small muted" style="margin-bottom:0">Restricted to administrators and the ' +
        'Information Officer.</p>') : '') +

      (canAudit ? card('Access log', 'The record of who saw what',
        '<p>Every ID number unlocked, document opened, and list exported, with the reason given ' +
        'and the name of whoever did it.</p>' +
        '<p>This is the evidence that ACTOM handles applicant information properly. It is ' +
        'append-only — nobody can edit or delete an entry, including administrators.</p>') : '') +

      card('Getting help', 'Who to ask',
        '<p><strong>The system itself</strong> — anything not working, a file that will not ' +
        'open, or a screen you expected to see: Group IT.</p>' +
        '<p><strong>Access and roles</strong> — a colleague who cannot sign in, or who needs ' +
        'a different level of access: an administrator activates accounts.</p>' +
        '<p><strong>Privacy questions</strong> — an applicant asking what is held about them, ' +
        'or asking for their data to be removed: the Information Officer, ' +
        '<a href="mailto:informationofficer@actom.co.za">informationofficer@actom.co.za</a>.</p>' +
        '<p class="small muted" style="margin-bottom:0">What has changed recently is under ' +
        '<strong>What\u2019s new</strong> at the bottom of the menu.</p>');
  }
  ...
  /* ==================================================== consent wording
     The Information Officer's screen. Wording becomes immutable the
     moment an applicant agrees to it — POPIA requires ACTOM to show what
     a person actually consented to, not what the paragraph says today.
     The database enforces that with a trigger; this screen explains it
     rather than letting someone discover it as an error. */

  function loadConsent() {
    sb.from('v_consent_versions').select('*')
      .order('version', { ascending: false }).order('audience')
      .then(function (r) {
        if (r.error) { $('content').innerHTML = errBox(r.error.message); return; }
        renderConsent(r.data || []);
      });
  }

  function renderConsent(rows) {
    var versions = [];
    rows.forEach(function (r) {
      if (versions.indexOf(r.version) < 0) versions.push(r.version);
    });

    $('content').innerHTML =
      '<div class="card"><div class="card-head"><div class="eyebrow">POPIA</div>' +
      '<h1>Consent wording</h1>' +
      '<p>What an applicant agrees to before submitting. Each version carries separate ' +
      'wording for the applicant and for the parent or guardian of an under-18.</p></div>' +

      '<div class="notice"><strong>Wording locks once it is used.</strong>' +
      'As soon as one applicant has agreed to a version, its text can no longer be changed \u2014 ' +
      'the record has to show what they actually saw. To change wording that is already in ' +
      'use, save it as a new version number.</div>' +

      '<div class="btn-row" style="margin-top:0">' +
      '<button class="btn" id="newVersionBtn">New version</button>' +
      '</div></div>' +

      versions.map(function (v) {
        var forV = rows.filter(function (r) { return r.version === v; });
        var used = forV.some(function (r) { return r.times_agreed > 0; });
        var inIntake = forV.some(function (r) { return r.used_by_intake; });

        return '<div class="card"><div class="card-head">' +
          '<h2 style="display:flex;align-items:center;gap:.5rem">Version ' + esc(v) +
          (used ? '<span class="tag tag-declined">locked</span>'
                : '<span class="tag tag-shortlisted">editable</span>') +
          (inIntake ? '<span class="tag tag-submitted">used by an intake</span>' : '') +
          '</h2>' +
          '<p>' + (used
            ? forV.reduce(function (n, r) { return n + r.times_agreed; }, 0) +
              ' applicant(s) have agreed to this version, so its wording is now part of the record.'
            : 'Not yet agreed to by anyone, so it can still be edited.') + '</p></div>' +

          forV.map(function (r) {
            return '<div style="margin-bottom:1rem">' +
              '<h3 style="text-transform:capitalize">' + esc(r.audience) + '</h3>' +
              '<div class="field">' +
              '<textarea data-body="' + esc(r.id) + '" rows="8"' +
              (r.editable ? '' : ' readonly style="background:var(--surface-soft)"') +
              '>' + esc(r.body) + '</textarea></div>' +
              '<div class="btn-row" style="margin-top:.3rem">' +
              (r.editable
                ? '<button class="btn btn-sm" data-save="' + esc(r.id) + '" ' +
                  'data-version="' + esc(r.version) + '" data-audience="' + esc(r.audience) + '">' +
                  'Save changes</button>'
                : '<span class="small muted" style="align-self:center">Read only \u2014 already agreed to</span>') +
              '<span class="spacer"></span>' +
              '<button class="btn btn-ghost btn-sm" data-active="' + esc(r.id) + '" ' +
              'data-to="' + (r.active ? 'false' : 'true') + '">' +
              (r.active ? 'Retire' : 'Reactivate') + '</button>' +
              '</div></div>';
          }).join('') + '</div>';
      }).join('');

    $('newVersionBtn').addEventListener('click', function () { newConsentDialog(rows); });

    $('content').querySelectorAll('[data-save]').forEach(function (b) {
      b.addEventListener('click', function () {
        var body = $('content').querySelector('[data-body="' + b.dataset.save + '"]').value;
        sb.rpc('save_consent_version', {
          p_version: b.dataset.version, p_audience: b.dataset.audience, p_body: body
        }).then(function (r) {
          if (r.error) return alert(r.error.message);
          if (!r.data.ok) return alert(r.data.reason);
          toast('Saved');
          loadConsent();
        });
      });
    ...
    $('content').querySelectorAll('[data-active]').forEach(function (b) {
      b.addEventListener('click', function () {
        sb.rpc('set_consent_version_active', {
          p_id: b.dataset.active, p_active: b.dataset.to === 'true'
        }).then(function (r) {
          if (r.error) return alert(r.error.message);
          toast(b.dataset.to === 'true' ? 'Reactivated' : 'Retired');
          loadConsent();
        });
      });
    });
  ...
  function newConsentDialog(existing) {
    // Offer to start from the newest existing wording, since a new
    // revision is almost always a small edit to the last one rather
    // than a blank page.
    var latest = {};
    existing.forEach(function (r) {
      if (!latest[r.audience] || r.version > latest[r.audience].version) latest[r.audience] = r;
    });

    var back = document.createElement('div');
    back.className = 'modal-backdrop';
    back.innerHTML =
      '<div class="modal" style="max-width:760px"><div class="modal-head"><div>' +
      '<div class="eyebrow">POPIA</div><h2>New consent version</h2></div>' +
      '<span style="flex:1"></span><button class="modal-close" aria-label="Close">&times;</button></div>' +

      '<div class="notice"><strong>Both audiences are required.</strong>' +
      'An intake cannot be published unless the version has wording for the applicant and ' +
      'for the guardian of an under-18. Publishing checks this.</div>' +

      '<div class="field"><label for="cvVersion">Version number <span class="req">*</span></label>' +
      '<input type="text" id="cvVersion" placeholder="e.g. 2026.2"></div>' +

      '<div class="field"><label for="cvApplicant">Applicant wording <span class="req">*</span></label>' +
      '<textarea id="cvApplicant" rows="8">' +
      esc(latest.applicant ? latest.applicant.body : '') + '</textarea></div>' +

      '<div class="field"><label for="cvGuardian">Guardian wording <span class="req">*</span></label>' +
      '<textarea id="cvGuardian" rows="6">' +
      esc(latest.guardian ? latest.guardian.body : '') + '</textarea></div>' +

      '<div id="cvProblem"></div>' +
      '<div class="btn-row"><button class="btn btn-ghost" id="cvCancel">Cancel</button>' +
      '<span class="spacer"></span>' +
      '<button class="btn" id="cvSave">Create version</button></div></div>';
    ...
    back.querySelector('#cvCancel').addEventListener('click', close);

    back.querySelector('#cvSave').addEventListener('click', function () {
      var btn = this;
      var version = back.querySelector('#cvVersion').value.trim();
      var appBody = back.querySelector('#cvApplicant').value.trim();
      var gdnBody = back.querySelector('#cvGuardian').value.trim();

      if (!version || !appBody || !gdnBody) {
        back.querySelector('#cvProblem').innerHTML =
          '<div class="notice notice-err">A version number and both wordings are required.</div>';
        return;
      }
      if (existing.some(function (r) { return r.version === version; })) {
        back.querySelector('#cvProblem').innerHTML =
          '<div class="notice notice-err">Version ' + esc(version) + ' already exists. ' +
          'Choose a new number.</div>';
        return;
      }

      btn.disabled = true; btn.textContent = 'Saving\u2026';
      // Applicant first, then guardian. If the second fails the first
      // still exists, which is recoverable and visible on screen —
      // better than a silent half-save with no record of either.
      sb.rpc('save_consent_version', {
        p_version: version, p_audience: 'applicant', p_body: appBody
      }).then(function (r1) {
        if (r1.error || !r1.data.ok) throw new Error(r1.error ? r1.error.message : r1.data.reason);
        return sb.rpc('save_consent_version', {
          p_version: version, p_audience: 'guardian', p_body: gdnBody
        });
      }).then(function (r2) {
        if (r2.error || !r2.data.ok) throw new Error(r2.error ? r2.error.message : r2.data.reason);
        close();
        toast('Version ' + version + ' created');
        loadConsent();
      }).catch(function (e) {
        btn.disabled = false; btn.textContent = 'Create version';
        back.querySelector('#cvProblem').innerHTML =
          '<div class="notice notice-err">' + esc(e.message) + '</div>';
      });
    });
  }
  ...
  function openApprentice(a) {
    ...
    $('content').innerHTML =
      ...
      // Employee and SETA numbers are added AFTER enrolment in practice:
      // the SETA issues a learner number only once the contract has been
      // registered, which is usually weeks after the person has started.
      // Without these here there was no way to record it later.
      '<div class="grid grid-2">' +
      '<div class="field"><label for="uEmp">Employee number</label>' +
      '<input type="text" id="uEmp" value="' + esc(a.employee_number) + '"></div>' +
      '<div class="field"><label for="uSeta">SETA learner number</label>' +
      '<input type="text" id="uSeta" value="' + esc(a.seta_learner_number) + '"></div></div>' +
      '<div class="grid grid-2">' +
    ...
    $('uSave').addEventListener('click', function () {
      sb.rpc('update_apprentice', {
        ...
        p_employee_number: $('uEmp').value.trim() || null,
        p_seta_number: $('uSeta').value.trim() || null,
  ...
  /* Advisory qualification reading (migration 016).

     This is a GUESS at what the applicant typed into a free-text box.
     It is deliberately kept away from auto_score and the ranking, and
     the wording below says so on the card, because a reviewer glancing
     at a number next to a name will otherwise assume it counted. If
     qual_source ever stops being 'parsed', revisit that wording rather
     than leaving it to imply more certainty than there is. */
  function qualificationCard(a) {
    if (!a.highest_qualification && !(a.qual_codes || []).length) return '';

    var codes = a.qual_codes || [];
    var unrecognised = !codes.length && !!a.highest_qualification;

    var body;
    if (unrecognised) {
      body = '<div class="notice notice-warn" style="margin-top:.6rem">' +
             '<strong>Not recognised</strong>' +
             esc(a.highest_qualification) +
             '<br><br>Nothing in this wording matched a known qualification. ' +
             'Read the certificate rather than the field.</div>';
    } else {
      body = '<dl class="kv">' +
             kv('As typed', a.highest_qualification) +
             kv('Read as', codes.join(', ')) +
             kv('Workbook points', a.qual_points == null
                 ? '—'
                 : a.qual_points + (a.qual_highest ? '  (' + a.qual_highest + ')' : '')) +
             '</dl>' +
             '<p class="muted small" style="margin-top:.5rem">' +
             esc(a.qual_note || '') +
             ' These points are not part of the score or the rank.</p>';
    }

    return '<div class="card"><div class="eyebrow">Qualification held</div>' + body + '</div>';
  }
