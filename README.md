...
shared/changelog.js    APP_VERSION and release notes — updated on EVERY change
...
test-version.js        version bumped and a release note written
DEPLOYMENT.md          step-by-step runbook — start here
```

Before anything else: `npm install && npm test`. Eleven suites, ~290 checks, no network
...
`shared/changelog.js` holds `APP_VERSION` and the release notes. Both are
updated on every change — the pre-commit hook refuses a commit that touches
`apps/`, `shared/` or `db/` without one, and `test-version.js` fails the build.

This is not bookkeeping. The change history of a quality system is something a
certification body asks to see, and reconstructing it from a git log months
later is not the same artefact. The notes are written for the person using the
system, so the suite rejects developer jargon in them.
