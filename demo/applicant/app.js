...
(function () {
  ...
  function stepTrade() {
    ...
    var cards = S.trades.map(function (t) {
      ...
      return '<button type="button" class="trade-card' +
        ...
        (t.division ? '<span class="tc-div">' + esc(t.division) + '</span>' : '') +
        '<h4>' + esc(t.name) + '</h4>' +
        (blurb ? '<p>' + esc(blurb) + '</p>' : '') +
    ...
    return '<div class="card">' +
      ...
      docTypes().map(function (d) {
        ...
        return '<div class="field"><span class="field-label">' + esc(d.label) +
          ...
          'accept=".pdf,.jpg,.jpeg,.png,.heic"></div>' +
  ...
  function handleUpload(docType, file) {
    ...
    file.slice(0, 32).arrayBuffer().then(function (head) {
      ...
      if (!real || !CFG.ACCEPTED_TYPES[real]) {
        errEl.textContent = 'We can only accept PDF, JPG, PNG or HEIC files. ' +
      ...
      return file.arrayBuffer().then(function (buf) {
        return crypto.subtle.digest('SHA-256', buf).then(function (h) {
          ...
          li.innerHTML = '<span class="name">' + esc(file.name) + '</span>' +
                         '<span class="meta">uploading…</span>';
          ...
          return sb.storage.from('applicant-documents')
            ...
            .catch(function (e) {
              li.remove();
              if (errEl) {
                errEl.textContent = 'The upload did not go through: ' +
                  (e && e.message ? e.message : 'unknown error') + '. Please try again.';
              }
              console.error('Upload failed', e);
            })
            .then(function () { uploading[docType] = false; });
        });
      });
    });
  }
  ...
  function renderSubmitted() {
    ...
    $('content').innerHTML =
      ...
      '<h1>You have applied, ' + esc((a.full_name || '').split(' ')[0]) + '</h1>' +
      '<p>That is the hard part done. Keep this reference and quote it in any email or call ' +
      'about your application.</p>' +
      '<p class="mono" style="font-size:1.6rem;letter-spacing:.06em;color:var(--blue)">' +
      esc(a.reference) + '</p>' +
      '<p class="small muted">Submitted ' +
      new Date(a.submitted_at).toLocaleString('en-ZA') + '</p>' +
      '</div>' +

      '<div class="card"><h2>What happens from here</h2>' +
      '<ul class="journey">' +
      '<li class="is-now"><strong>Application received</strong><span>Today. Nothing more is needed from you right now.</span></li>' +
      '<li><strong>Screening</strong><span>After the intake closes on ' +
        new Date(S.intake.closes_at).toLocaleDateString('en-ZA', { day: 'numeric', month: 'long', year: 'numeric' }) +
        '. We check every application against the requirements for your trade.</span></li>' +
      '<li><strong>Aptitude assessment</strong><span>If you are shortlisted we phone you on the number you gave us to arrange it.</span></li>' +
      '<li><strong>Interview and medical</strong><span>A conversation about the work, and a fitness-for-duty check.</span></li>' +
      '<li><strong>Contract of apprenticeship</strong><span>Signed and registered with the SETA. You start earning.</span></li>' +
      '<li><strong>Three years of training</strong><span>Workshop, site and classroom, working towards your trade test.</span></li>' +
      '<li><strong>Qualified artisan</strong><span>A national trade certificate, and a skill that travels.</span></li>' +
      '</ul>' +
      '<p class="small muted" style="margin-top:1rem">We contact everyone either way, so there is no need to follow up. ' +
      'We keep your application on file for 12 months after the intake closes in case a suitable ' +
      'position opens sooner, then it is deleted automatically.</p></div>' +
      ...
      '<button class="btn btn-ghost btn-sm" id="withdrawBtn" style="color:var(--err)">Withdraw my application</button></div>' +
